...
def drawLines(img,kpt1,kpt2):
    for i in range(len(kpt1)):
        ...
        i+=1
    cv2.imwrite("./result/nnnn.jpg",cv2.cvtColor(img,cv2.COLOR_RGB2BGR))
...
if __name__ == '__main__':
    ...
    match = knn.kneighbors(discriptors2,n_neighbors=2,return_distance=True)
    ...
    print("过滤前的特征点个数："+str(len(match[0])))
    ...
    match_dis=[]
    for i in range(X1.size):
        if match[0][i,0]<=match[0][i,1]*0.8:
            match1_dis.append([match[0][i,0]])
            # kpt2_index.append(match[1][i,0])
            # kpt1_index.append(i)
            index.append(i)
            X1_good.append(X1[i])
            Y1_good.append(Y1[i])
    ...
    for j in range(len(match1_dis)):
        match_dis.append(match1_dis[j][0])
        # idx1 = kpt1_index[j]

    # for k in range(len(kpt2_index)):
    #     idx2=kpt2_index[k]
    #     X2_good.append(X2[idx2])
    #     Y2_good.append(Y2[idx2])
    ...
    dis=list(match_dis)
    info = [list(t) for t in zip(X1,X2,Y1,Y2,dis)]
